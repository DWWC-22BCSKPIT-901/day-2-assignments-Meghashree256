...
public:
int res;
void backtrack(vector<int>& jobs, vector<int>& workerTime, int idx, int k, int
curMax) { if (idx == (int)jobs.size()) {
res = min(res, curMax);
return;
}
...
backtrack(jobs, workerTime, idx + 1, k, max(curMax, workerTime[i])); workerTime[i] -= jobs[idx];
...
cout << sol.maxHappyGroups(batchSize, groups) <<
endl; }
{
Solution sol;
int batchSize = 4;
...
int k = 3;
cout << sol2.minimumTimeRequired(jobs, k) <<
endl; }
{
Solution2 sol2;
